 ...
 **/
  class Model{
    ...
    public function read_filter_once($table, $field, $sfield, $value)
		...
		public function read_filter_and($table, $field, $sfield1, $sfield2, $field3='', $values) {

			if ($field3!='') {

				// get and return a database object
				$db = $this -> conn();
				$read_request = $db -> prepare('SELECT '.$field.' FROM '.$table.' WHERE '.$sfield1.'=? AND '.$sfield2.'=? and '.$field3.'=?');
				$read_request -> execute($values);
				return $read_request;
			} 
			else {
		...
		public function delete ($table, $field1, $field2='', $values) {
			...
			if ($field2 != '') {
				...
				$delete_request = $db -> prepare('DELETE FROM '.$table.' WHERE '.$field1.'=? AND '.$field2.'=?');
				$delete_request -> execute($values);
				//return $delete_request;

			} else {

				$delete_request = $db -> prepare('DELETE FROM '.$table.' WHERE '.$field1.'=?');
				$delete_request -> execute($values);
				//return $delete_request;
			}
